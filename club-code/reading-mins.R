...
reading <- Minutes.played.1920 %>% filter(team.id=="10")

reading <- reading %>% mutate_at(vars(mp.percentage), funs(round(., 0)))

ggplot(data = reading, aes(x = age, y = mp.percentage)) + 
  xlim(c(xmin, xmax)) + ylim(c(ymin, ymax)) +
  geom_point(colour = club_colour) +
  geom_text_repel(data = reading %>% filter(new.signing=="0"),
                  aes(label = player),
                  size = 3, family = "open-sans",
                  point.padding = 0.2) +
  geom_text_repel(data = reading %>%
  ...
  labs(title = paste0(club_name, " squad profile 2020/21 season"),
       subtitle = paste0("Average age: ", round(mean(reading$age), 0)),
