...
everton <- Minutes.played.1920 %>% filter(team.id=="7")

everton <- everton %>% mutate_at(vars(mp.percentage), funs(round(., 0)))

christiansen <- data.frame("Everton", 7, "Izzy Christansen", "MF", 0, 0, 28, 0)
names(christiansen) <- c("team", "team.id", "player", "position", "minutes.played", "mp.percentage", "age", "new.signing")
everton <- rbind(everton, christiansen)

ggplot(data = everton, aes(x = age, y = mp.percentage)) + 
  xlim(c(xmin, xmax)) + ylim(c(ymin, ymax)) +
  geom_point(colour = club_colour) +
  geom_text_repel(data = everton %>% filter(new.signing=="0"),
                  aes(label = player),
                  size = 3, family = "open-sans",
                  point.padding = 0.2) +
  geom_text_repel(data = everton %>%
  ...
  labs(title = paste0(club_name, " squad profile 2020/21 season"),
       subtitle = paste0("Average age: ", round(mean(everton$age), 0)),
