...
bristol <- Minutes.played.1920 %>% filter(team.id=="5")

bristol <- bristol %>% mutate_at(vars(mp.percentage), funs(round(., 0)))

ggplot(data = bristol, aes(x = age, y = mp.percentage)) + 
  xlim(c(xmin, xmax)) + ylim(c(ymin, ymax)) +
  geom_point(colour = club_colour) +
  geom_text_repel(data = bristol %>% filter(new.signing=="0"),
                  aes(label = player),
                  size = 3, family = "open-sans",
                  point.padding = 0.2) +
  geom_text_repel(data = bristol %>%
  ...
  labs(title = paste0(club_name, " squad profile 2020/21 season"),
       subtitle = paste0("Average age: ", round(mean(bristol$age), 0)),
