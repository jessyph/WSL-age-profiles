...
club_colour = "#0E44D3"
club_shade = "#B2B2AC"
club_name = "Birmingham City"

birmingham <- Minutes.played.1920 %>% filter(team.id=="3")
...
ggplot(data = birmingham, aes(x = age, y = mp.percentage)) + 
  ...
  geom_text_repel(data = birmingham %>% filter(new.signing=="0"),
                  aes(label = player),
                  size = 3, family = "open-sans",
                  point.padding = 0.2) +
  geom_text_repel(data = birmingham %>%
  ...
  labs(title = paste0(club_name, " squad profile 2020/21 season"),
       subtitle = paste0("Average age: ", round(mean(birmingham$age), 0)),
