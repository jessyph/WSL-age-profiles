...
brighton <- Minutes.played.1920 %>% filter(team.id=="4")

brighton <- brighton %>% mutate_at(vars(mp.percentage), funs(round(., 0)))

ggplot(data = brighton, aes(x = age, y = mp.percentage)) + 
  xlim(c(xmin, xmax)) + ylim(c(ymin, ymax)) +
  geom_point(colour = club_colour) +
  geom_text_repel(data = brighton %>% filter(new.signing=="0"),
                  aes(label = player),
                  size = 3, family = "open-sans",
                  point.padding = 0.2) +
  geom_text_repel(data = brighton %>%
  ...
  labs(title = paste0(club_name, " squad profile 2020/21 season"),
       subtitle = paste0("Average age: ", round(mean(brighton$age), 0)),
