...
arsenal <- arsenal %>% mutate_at(vars(mp.percentage), funs(round(., 0)))

arsenal <- arsenal[-c(3), ]

ggplot(data = arsenal, aes(x = age, y = mp.percentage)) + 
  xlim(c(xmin, xmax)) + ylim(c(ymin, ymax)) +
  geom_point(colour = club_colour) +
  geom_text_repel(data = arsenal %>% filter(new.signing=="0"),
                  aes(label = player),
                  size = 3, family = "open-sans") +
                  #point.padding = 0.3,
                  #label.padding = 0.2) +
  geom_text_repel(data = arsenal %>%
  ...
  labs(title = paste0(club_name, " squad profile 2020/21 season"),
       subtitle = paste0("Average age: ", round(mean(arsenal$age), 0)),
